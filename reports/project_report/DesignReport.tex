...
\textbf{Test 1:} Does the optimization correctly fail if the intervention time is greater than the final time?\vspace{3mm}\\
\textbf{Test 2:} Does the optimization correctly fail if the total resources are too large?\vspace{3mm}\\
...
\textbf{Alta Fang:}\\
\hangindent=1cm{Wrapped the optimization around the stochastic solver and handled the user inputs. Generated the primary user interface. Made the software distributable and installable through distutils. Wrote Unittests for the optimization code.}\\
